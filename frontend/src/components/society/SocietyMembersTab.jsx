...
const ROLE_CONFIG = {
  super_admin: {
    label: "Super Admin",
    color:
      "text-white bg-gradient-to-r from-amber-500 to-orange-500 ring-2 ring-amber-500/30",
    icon: Crown,
  },
  society_admin: {
    label: "Admin",
...
const SocietyMembersTab = ({ societyId, userRole }) => {
  ...
  const canActOn = (targetMember) => {
    ...
    if (
      targetMember.user?._id === user?.id ||
      targetMember.user?._id === user?._id
    )
      return false;
  ...
  return (
    <div>
          ...
          <div>
            ...
            <div className="text-xs text-gray-600 dark:text-gray-400 space-y-1">
              ...
              <p>
                ...
                Can manage members, events, and announcements
              </p>
              <p>
      ...
      {/* Members List */}
      <div className="bg-white dark:bg-[#1a1a2e] border border-gray-200 dark:border-gray-800/50 rounded-xl">
            ...
            return (
              <div
                ...
                className={`flex items-center gap-4 px-5 py-3 transition-colors ${
                  isMe
                    ? "bg-amber-500/5"
                    : "hover:bg-gray-50 dark:hover:bg-[#111118]"
                }`}
              >
                {/* Avatar */}
                <ProfileLink
                  user={member.user}
                  className="w-9 h-9 rounded-full bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center text-white text-sm font-bold flex-shrink-0"
                >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <ProfileLink
                      user={member.user}
                      className="text-sm font-medium text-gray-900 dark:text-white hover:text-amber-500 truncate transition-colors"
                    >
                ...
                {/* Role Badge */}
                <span
                  className={`hidden sm:flex items-center gap-1 text-[11px] px-2 py-1 rounded-full font-medium ${cfg.color}`}
                >
                ...
                {showActions && (
                  <div className="relative">
                    <button
                      onClick={() =>
                        setActionMenu(
                          actionMenu === member._id ? null : member._id,
                        )
                      }
                    ...
                    {actionMenu === member._id && (
                      <>
                        ...
                        <div className="absolute right-0 top-full mt-1 w-48 bg-white dark:bg-[#1e1e30] border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl z-20 py-1">
                          {/* Role change */}
                          {myRoleIndex >=
                            ROLE_HIERARCHY.indexOf("society_admin") && (
                            <>
                              ...
                              {ROLE_HIERARCHY.filter(
                                (r) =>
                                  r !== "super_admin" &&
                                  r !== member.role &&
                                  ROLE_HIERARCHY.indexOf(r) < myRoleIndex,
                              ).map((r) => (
                                <button
                                  key={r}
                                  onClick={() =>
                                    handleAction(`role:${r}`, member)
                                  }
                          ...
                          <button
                            onClick={() =>
                              setConfirmAction({ type: "kick", member })
                            }
                          ...
                          <button
                            onClick={() =>
                              setConfirmAction({ type: "ban", member })
                            }
      ...
      {confirmAction && (
        ...
        >
          <div
            className="bg-white dark:bg-[#1e1e30] rounded-xl p-6 w-full max-w-sm"
          ...
          >
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-5">
              ...
              <strong>
                <ProfileLink
                  user={confirmAction.member.user}
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                onClick={() =>
                  handleAction(confirmAction.type, confirmAction.member)
                }
